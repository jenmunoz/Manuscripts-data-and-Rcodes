#######################################################################################
### Manuscript-parasites and flocking species                                     ###
...
# Data summaries ---------------------------------------------------
# Load and clean data
ectos_birds_dff <- read.csv("data/data_manuscript/3_dff_all_ectos_prevalence_abundance_diversity_individual_elevation_mass_FILE_TIDY.csv", 
                            na.strings = c("", "NA")) %>%
  select(general_diversity, family, elevation, species_jetz, Powder.lvl, ectoparasites_PA, foraging_cat, sociality, year_seasonality, 
         mass_tidy_species, mass_ind_tidy, mass_ind_comp) %>%
  na.omit()
...
# Summarise ectoparasites presence
a <- ectos_birds_dff %>%
  group_by(species_jetz) %>%
  summarise(ectoparasites_presence = (sum(ectoparasites_PA)), 
            sample_size = (n()), 
            sociality = max(sociality))

a %>%
  group_by(sociality) %>%
  summarise(n = n(), P = (n() / 871 * 100))

# Distinct ectoparasites presence and filter by sample size
ectos_birds_dff <- ectos_pres_abs %>%
  distinct(species_jetz, proportion_ectoparasites, .keep_all = TRUE) %>%
...
# Calculate prevalence ensuring families have more than 5 samples
prevalence <- prevalence_step1 %>%
  summarise(family_name = first(family), 
            total_samples = sum(n), 
            total_positive = last(n), 
            prevalence = ((total_positive / total_samples) * 100)) %>%
...
# Abundance by group: Mites
ectos_birds_dff <- read.csv("data/data_manuscript/3_dff_all_ectos_prevalence_abundance_diversity_individual_elevation_mass_FILE_TIDY.csv", 
                            na.strings = c("", "NA")) %>%
  select(general_diversity, family, elevation, species_jetz, Powder.lvl, ectoparasites_PA, foraging_cat, sociality, year_seasonality, 
         mass_tidy_species, mass_ind_tidy, mass_ind_comp, total_mites, total_no_feathers_mites) %>%
  na.omit()

ectos_birds_dff %>%
  summarize(max = max(total_mites), mean = mean(total_mites), sd = sd(total_mites), min = min(total_mites))

ectos_birds_dff %>%
  summarize(max = max(total_no_feathers_mites), mean = mean(total_no_feathers_mites), sd = sd(total_no_feathers_mites), min = min(total_no_feathers_mites))

ectos_birds_dff %>%
  filter(total_mites != 0) %>%
  summarise(samples_mites = sum(!is.na(total_mites)), p = samples_mites / 831 * 100)

# Abundance by group: Lice
ectos_birds_dff <- read.csv("data/data_manuscript/3_dff_all_ectos_prevalence_abundance_diversity_individual_elevation_mass_FILE_TIDY.csv", 
                            na.strings = c("", "NA")) %>%
  select(general_diversity, family, elevation, species_jetz, Powder.lvl, ectoparasites_PA, foraging_cat, sociality, year_seasonality, 
         mass_tidy_species, mass_ind_tidy, mass_ind_comp, total_lice) %>%
  na.omit()

ectos_birds_dff %>%
  summarize(max = max(total_lice), mean = mean(total_lice), sd = sd(total_lice), min = min(total_lice))

ectos_birds_dff %>%
  filter(total_lice != 0) %>%
  summarise(samples_lice = sum(!is.na(total_lice)), p = samples_lice / 831 * 100, max = max(total_lice))

# Lice diversity (Requested by Jill)
lice_species_dff <- read.csv("data/data_manuscript/7.lice_df_diversity_species.csv", na.strings = c("", "NA")) %>%
  na.omit()

lice_genus_dff <- read.csv("data/data_manuscript/7.lice_df_diversity_genus.csv", na.strings = c("", "NA"))

# Summarise lice species diversity
lice_species_dff %>%
  group_by(species_clean) %>%
  summarise(unique_lice = n_distinct(valuecol), .groups = 'drop')

# Summarise lice genus diversity
lice_genera <- lice_genus_dff %>%
  group_by(species_jetz) %>%
  summarise(unique_count = n_distinct(valuecol))

# View lice genus data
View(lice_genera)

# Calculate diversity percentage
diversity_lice_percentage <- lice_genera %>%
  group_by(unique_count) %>%
  summarise(count = n(), percentage = (count / 103) * 100)

# Abundance by group: Ticks
ectos_birds_dff <- read.csv("data/data_manuscript/3_dff_all_ectos_prevalence_abundance_diversity_individual_elevation_mass_FILE_TIDY.csv", 
                            na.strings = c("", "NA")) %>%
  select(general_diversity, family, elevation, species_jetz, Powder.lvl, ectoparasites_PA, foraging_cat, sociality, year_seasonality, 
         mass_tidy_species, mass_ind_tidy, mass_ind_comp, Ticks) %>%
  na.omit()
...
unique(ectos_birds_dff$total_lice)
#%>% filter(total_lice<60)  # removing outliers 
...
dff_ectos_network_individual_metrics<-read.csv("data/data_manuscript/3_dff_all_ectos_network_metrics_individuals_FILE_TIDY.csv",na.strings =c("","NA"))%>% 
  select(elevation, species_jetz, Powder.lvl,foraging_cat, sociality,ectoparasites_PA, degree, w_degree, year_seasonality, mass_tidy_species, mass_ind_tidy, mass_ind_comp,interspecifically_gregarious_exploratory) %>% 
  na.omit() %>% filter(species_jetz!="Premnoplex_brunnescens")  #Removing outliers for total mites
...
dff_ectos_network_individual_metrics$interspecifically_gregarious_exploratory<-as.factor(dff_ectos_network_individual_metrics$interspecifically_gregarious_exploratory)
...
selected_ecto_p_brms_bayes_no_int_degree_prior_gregarious<-brms::brm(ectoparasites_PA~scale(degree)+ scale(elevation)+ scale(year_seasonality)+ 
                                                            scale(mass_tidy_species)+(mass_ind_comp)+(interspecifically_gregarious_exploratory)+
